...
Call KillingDraw(Pic1, pt_, vbWhite)
...
For j = LineNum - MoveStep To 1 Step -1
    For i = 0 To 11
        If (Coordinates(i, j) <> 0 And j <> 0) Then
            ...
            apt.ptX = i
            apt.ptY = j
            Call TranslatingDraw(Pic1, apt, vbWhite)
            
            apt.ptY = j + MoveStep
            Call TranslatingDraw(Pic1, apt, ColorB2L(Coordinates(i, j)))
            Coordinates(i, j) = 0
        End If
    Next
Next
...
For i = 0 To DLX - 1
    For j = 0 To DLY - 1
        dp.ptX = i
        dp.ptY = j
        If Coordinates(i, j) <> 0 Then TranslatingDraw Pic1, dp, ColorB2L(Coordinates(i, j))
