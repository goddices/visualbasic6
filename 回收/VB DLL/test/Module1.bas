...
Public Sub CreateBar(Object As PictureBox, spt As Point, thisBar As Bar, Optional isDown As Boolean, Optional isNext As Boolean)        'As Point
    ...
    Y4 = spt.ptY + 4                          ' □
    '---------------
    Dim bColorCode As Byte, color As Long
    ...
    If Not isNext Then
        For i = 0 To 7 Step 2
            ...
            Coordinates(ptPointsContainer(i), ptPointsContainer(i + 1)) = bColorCode
        Next
    End If
...
Public Sub TranslatingDraw(Object As Object, pt1 As Point, color As Long)
    Dim Range As RECT
    hBrush = CreateSolidBrush(color)
