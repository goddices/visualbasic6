...
Public Function GetScreen(PNT As D3DVECTOR, Optional Xpos As Single, Optional YPOS As Single) As D3DVECTOR
 Dim Vtmp As D3DVECTOR4, vout As D3DVECTOR4
 Vtmp = vec3Tovec4(PNT)
 D3DXVec4Transform vout, Vtmp, matlast
 If vout.w = 0 Then vout.w = 0.00001
 Xpos = (vout.X / vout.w + 1) / 2 * (d3dsdBackBuffer.Width)
 YPOS = (1 - vout.Y / vout.w) / 2 * (d3dsdBackBuffer.Height)
 GetScreen.X = Xpos: GetScreen.Y = YPOS: GetScreen.Z = 0
End Function
